...
\usepackage{float}
\usepackage{xurl}
\usepackage{afterpage} % para páginas em branco
...
% Comando auxiliar para o \paginaembranco
\newcommand\blankpage{%
    \null
    \thispagestyle{empty}%
    \addtocounter{page}{-1}%
...
% Cria uma página em branco SEM NUMERAÇÃO após completar a atual
% (assim que é possível)
\newcommand\paginaembranco{%
    \afterpage{\blankpage}
    }
...
\begin{figure}[p]
  %\begin{center}
    %\hspace{1.7cm}
    \includegraphics[height=1\textheight]{img/capa_2023.png} %REFTIME
  %\end{center}
\end{figure}
